...
contract Lending is ILending, Ownable {
    ...
    function depositCollateral(uint256 amount) external onlyOwner {
        ...
        if (!collateralToken.transferFrom(owner(), address(this), amount)) revert TRANSFER_FAILED();

        // Update owner's collateral balance
        users[owner()].collateral += amount;
    ...
    function borrow(uint256 amount) external onlyOwner {
        ...
        if (users[owner()].collateral * collateralRatio < users[owner()].borrowedAmount + amount) revert INSUFFICIENT_COLLATERAL();
        ...
        emit FundsBorrowed(owner(), amount);

        // Update owner's borrowed amount
        users[owner()].borrowedAmount += amount;
    }
     ...
     */
    function calculateInterest() external onlyOwner returns (uint256) {
        uint256 utilizationRate =
            users[owner()].borrowedAmount * 1e18 / users[owner()].collateral;

        // Calculate time elapsed since last interest accrual
        uint256 timeElapsed = block.timestamp - lastInterestAccrualTime[owner()];

        // Update the last interest accrual time
        lastInterestAccrualTime[owner()] = block.timestamp;
        ...
        if (utilizationRate <= optimalUtilizationRate) {
            ...
            interestAccrued = (users[owner()].borrowedAmount * interestRate * timeElapsed) / (1e18 * 365 days);
        } else {
            // Above optimal utilization rate, charge a constant rate
            interestAccrued = (users[owner()].borrowedAmount * excessUtilizationRate * timeElapsed) / (1e18 * 365 days);
        }
    ...
    function repayLoan(uint256 amount) external onlyOwner {
        ...
        if (collateralToken.balanceOf(owner()) < amount) revert INSUFFICIENT_BALANCE();
     ...
     */
    function liquidatePosition(address borrower) external onlyOwner {
    ...
    function withdrawTokens(uint256 amount) external onlyOwner {
        ...
        if (!collateralToken.transfer(owner(), amount)) revert TRANSFER_FAILED();
